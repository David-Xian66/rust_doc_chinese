"""Translate 4 untranslated docblocks in quinn/congestion/struct.NewRenoConfig.html."""

import re

PATH = 'D:/Administrator/Documents/Code/rust_doc_all/quinn/congestion/struct.NewRenoConfig.html'

TRANSLATIONS = [
    ('<p>Default limit on the amount of outstanding data in bytes.</p>\n<p>Recommended value: <code>min(10 * max_datagram_size, max(2 * max_datagram_size, 14720))</code></p>',
     '<p>默认的、在途未确认数据字节数上限。</p>\n<p>推荐值：<code>min(10 * max_datagram_size, max(2 * max_datagram_size, 14720))</code></p>'),
    ('<p>Reduction in congestion window when a new loss event is detected.</p>',
     '<p>检测到新的丢包事件时，拥塞窗口的缩减比例。</p>'),
