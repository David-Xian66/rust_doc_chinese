"""Translate 6 untranslated docblocks in quinn/struct.StreamId.html."""

import re

PATH = 'D:/Administrator/Documents/Code/rust_doc_all/quinn/struct.StreamId.html'

TRANSLATIONS = [
    ('<p>Create a new StreamId</p>',
     '<p>创建一个新的 StreamId</p>'),
    ('<p>Which side of a connection initiated the stream</p>',
     '<p>连接的哪一方发起了该流</p>'),
    ('<p>Which directions data flows in</p>',
     '<p>数据的流动方向</p>'),
    ('<p>Distinguishes streams of the same initiator and directionality</p>',
     '<p>用于区分相同发起方与相同方向上的不同流</p>'),
