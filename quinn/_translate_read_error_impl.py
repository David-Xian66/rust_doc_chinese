"""Translate 7 untranslated docblocks in quinn/enum.ReadError.html."""

import re

PATH = 'D:/Administrator/Documents/Code/rust_doc_all/quinn/enum.ReadError.html'

TRANSLATIONS = [
    ('<p>The peer abandoned transmitting data on this stream</p>\n<p>Carries an application-defined error code.</p>',
     '<p>对端放弃在该流上继续发送数据</p>\n<p>携带一个由应用层定义的错误码。</p>'),
    ('<p>The connection was lost</p>',
     '<p>连接已丢失</p>'),
    ('<p>The stream has already been stopped, finished, or reset</p>',
     '<p>该流已经被 stop、finish 或 reset</p>'),
    ('<p>Attempted an ordered read following an unordered read</p>\n<p>Performing an unordered read allows discontinuities to arise in the receive buffer of a\nstream which cannot be recovered, making further ordered reads impossible.</p>',
     '<p>在一次无序读取之后又尝试了一次有序读取</p>\n<p>无序读取会在流的接收缓冲区中产生无法恢复的不连续，从而无法再进行有序读取。</p>'),
