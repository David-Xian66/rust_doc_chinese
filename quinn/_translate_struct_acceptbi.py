"""Translate quinn/struct.AcceptBi.html to Chinese.

struct.AcceptBi is a Future produced by Connection::accept_bi. Same pattern
as struct.Accept.
"""
...
def translate_struct_acceptbi(content):
    ...
    content = content.replace(
        '<title>AcceptBi in quinn - Rust</title>',
        '<title>quinn 中的 AcceptBi - Rust</title>',
    )
    content = content.replace(
        '<meta name="description" content="Future produced by `Connection::accept_bi`">',
        '<meta name="description" content="由 `Connection::accept_bi` 产生的 Future">',
    )
    content = content.replace(
        '<h1>Struct <span class="struct">Accept<wbr>Bi</span>',
        '<h1>结构体 <span class="struct">Accept<wbr>Bi</span>',
    )
    content = content.replace(
        '<p>Future produced by <a href="struct.Connection.html#method.accept_bi" title="method quinn::Connection::accept_bi"><code>Connection::accept_bi</code></a></p>',
        '<p>由 <a href="struct.Connection.html#method.accept_bi" title="method quinn::Connection::accept_bi"><code>Connection::accept_bi</code></a> 产生的 Future</p>',
    )
    return content


def main():
    path = os.path.join(QUINN_ROOT, 'struct.AcceptBi.html')
    print(f'--- {path} ---')
    with open(path, 'r', encoding='utf-8') as f:
        content = f.read()
    new_content = translate_struct_acceptbi(content)
    with open(path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    rel = os.path.relpath(path, QUINN_ROOT)
    verify(new_content, rel)
    for ident in ['AcceptBi', 'Connection', 'Future', 'quinn::Connection::accept_bi',
                  '<span class="struct">Accept<wbr>Bi</span>',
                  'struct.Connection.html#method.accept_bi']:
        if ident not in new_content:
            print(f'  [WARN] identifier missing: {ident!r}')
    for needle in ['quinn 中的 AcceptBi', '由 `Connection::accept_bi` 产生的 Future',
                   '结构体 <span class="struct">Accept<wbr>Bi</span>',
